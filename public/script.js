...
let sessionTimeout
// let submittedSurveys = new Set() // Track which surveys have been submitted
...
window.submitSurvey = async (event, surveyId) => {
  ...
  try {
    ...
    if (response.ok) {
      // Ensure submittedSurveys is always available on window object
      if (!window.submittedSurveys) {
        window.submittedSurveys = new Set()
      }
      ...
      // Get the total number of available surveys first
...
async function displayAvailableSurveys() {
  try {
    ...
    if (availableSurveys.length === 0) {
      // Show the success message instead of "No new surveys available"
      container.innerHTML = `
        <div class="success-message">
          <h4><i class="fas fa-check-circle"></i> Survey Submitted Successfully!</h4>
          <p>You can logout now.</p>
...
window.navigateSurvey = (direction) => {
  // Ensure submittedSurveys is always available
  if (!window.submittedSurveys) {
    window.submittedSurveys = new Set()
  }

  // Get current survey info
  const currentSurveyCard = document.querySelector("#available-surveys .survey-card")
  const currentForm = currentSurveyCard.querySelector("form")
  const currentSurveyId = currentForm.getAttribute("onsubmit").match(/'([^']+)'/)[1]

  // Check if current survey is submitted when trying to go to next
  if (direction === "next" && !window.submittedSurveys.has(currentSurveyId)) {
    alert("Please submit this survey before proceeding to the next one.")
    return
  }

  // Check if all questions in current survey are answered (existing logic)
  ...
  if (!allAnswered && direction === "next") {
    alert(`Please answer all questions before proceeding. Unanswered questions: ${unansweredQuestions.join(", ")}`)
...
function generateQuestionInput(question, index, surveyColor = "#253074") {
  ...
  switch (question.type) {
    case "text":
      inputHtml = `<div class="input-field-container"><textarea name="q${index}" class="response-input" rows="5" cols="150"></textarea></div>`
      break
    case "radio":
      inputHtml = `
              ...
              ${question.options
                .map(
                  ...
                  <div class="radio-button">
                      <input type="radio" id="q${index}_${option}" name="q${index}" value="${option}">
      ...
      inputHtml = `
        <div class="star-rating-container">
            ${[1, 2, 3, 4, 5]
              .map(
                (star) => `
                <div class="star-rating-option" data-rating="${star}">
                    <input type="radio" id="q${index}_${star}" name="q${index}" value="${star}">
                    <label for="q${index}_${star}">★</label>
                </div>
            `,
              )
              .join("")}
...
async function loadAvailableSurveys() {
  try {
    // Ensure submittedSurveys is always available and clear it when loading new surveys
    if (!window.submittedSurveys) {
      window.submittedSurveys = new Set()
    } else {
      // Clear submitted surveys when loading new surveys
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.body.addEventListener("click", (e) => {
    if (e.target.closest(".star-rating-option")) {
      ...
      const starContainer = starOption.closest(".star-rating-container")
      const allStars = starContainer.querySelectorAll(".star-rating-option")
      const input = starOption.querySelector("input")
      const rating = Number.parseInt(starOption.dataset.rating)

      // Reset all stars to default color
      allStars.forEach((star) => {
        star.classList.remove("selected")
        star.querySelector("label").style.color = "#ddd"
      })

      // Color selected stars with survey color
      const surveyColor = getComputedStyle(document.documentElement).getPropertyValue("--survey-color") || "#253074"

      for (let i = 0; i < rating; i++) {
        allStars[i].classList.add("selected")
        allStars[i].querySelector("label").style.color = surveyColor
      }
...
function generateSurveyColumns(questions, surveyColor) {
  // Check if all questions are text type
  const allTextQuestions = questions.every((q) => q.type === "text")

  if (allTextQuestions) {
    // For all text questions, display them in a single column without separators
    return `
      <div class="survey-question-column" style="width: 100%; border: none;">
          ${questions.map((question, index) => generateQuestionInput(question, index, surveyColor)).join("")}
      </div>
    `
  }

  // For mixed question types, use the existing column layout
  ...
  questions.forEach((question, index) => {
    ...
    if (currentColumn.length === questionsPerColumn || index === questions.length - 1) {
      columns.push(currentColumn)
      currentColumn = []
    }
  })

  // Ensure we always have at least 2 columns for separator to work (except for all text)
